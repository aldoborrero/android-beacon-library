...
/**
 * This class provides relief for Android Bug 67272.  This bug in the Bluedroid stack causes
 * crashes in Android's BluetoothService when scanning for BLE devices encounters a large number of
 * unique devices.
 * It is rare for most users but can be problematic for those with apps scanning for
...
public class BluetoothCrashResolver {
  private static final String TAG = BluetoothCrashResolver.class.getSimpleName();
  ...
  private void startRecovery() {
    ...
    if (!adapter.isDiscovering()) {
      ...
      if (TIME_TO_LET_DISCOVERY_RUN_MILLIS > 0) {
        LogManager.d(TAG, "We will be cancelling this discovery in %s milliseconds.",
      ...
      if (action.equals(BluetoothAdapter.ACTION_STATE_CHANGED)) {
        ...
        switch (state) {
          ...
          case BluetoothAdapter.STATE_ON:
            ...
            LogManager.d(TAG, "Bluetooth was turned off for %s milliseconds",
                lastBluetoothTurningOnTime - lastBluetoothOffTime);
            if (lastBluetoothTurningOnTime - lastBluetoothOffTime
                < SUSPICIOUSLY_SHORT_BLUETOOTH_OFF_INTERVAL_MILLIS) {
              crashDetected();
            }
            break;
  ...
  private void saveState() {
    ...
    try {
      outputStream =
          context.openFileOutput(DISTINCT_BLUETOOTH_ADDRESSES_FILE, Context.MODE_PRIVATE);
