...
/**
 * Class allowing a user to set up background launching of an app when a user enters a beacon
 * Region.
 * Simply constructing and holding a reference to this class will cause background scanning for
 * beacons
 ...
 * a second MonitorNotifier, otherwise it will unregister the one configured by the
 * RegionBootstrap,
 * effectively disabling it.  When using the RegionBootstrap, any custom monitoring code must
 * therefore be placed in the callback methods in the BootstrapNotifier implementation passed to
 * the
 * RegionBootstrap.
 */
public class RegionBootstrap {
  ...
  public RegionBootstrap(BootstrapNotifier application, Region region) {
    if (application.getApplicationContext() == null) {
      throw new NullPointerException(
          "The BootstrapNotifier instance is returning null from its getApplicationContext() method.  Have you implemented this method?");
    }
  ...
  /**
   * Used to disable additional bootstrap callbacks after the first is received.  Unless this is
   * called,
  ...
  private class InternalBeaconConsumer implements BeaconConsumer {
     ...
     * Method reserved for system use
     */
    @Override public boolean bindService(Intent intent, ServiceConnection conn, int arg2) {
