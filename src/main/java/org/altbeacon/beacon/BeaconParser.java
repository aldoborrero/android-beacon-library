 ...
 * via subclassing ({@link AltBeaconParser see AltBeaconParser}) or by simply constructing an
...
public class BeaconParser {
   ...
   * </pre>
   *
   * <p>Each prefix is followed by a colon, then an inclusive decimal byte offset for the field
   * from
   ...
   * which
   * indicates the field should be parsed as little endian.  If not present, the field will be
   * presumed
  ...
  @TargetApi(5)
  protected Beacon fromScanData(byte[] bytesToProcess, int rssi, BluetoothDevice device,
      Beacon beacon) {
    ...
    for (Pdu pdu : advert.getPdus()) {
      if (pdu.getType() == Pdu.GATT_SERVICE_UUID_PDU_TYPE
        ...
        if (LogManager.isVerboseLoggingEnabled()) {
          LogManager.d(TAG, "Processing pdu type %02X: %s with startIndex: %d, endIndex: %d",
              pdu.getType(), bytesToHex(bytesToProcess), pdu.getStartIndex(), pdu.getEndIndex());
        }
        break;
      } else {
        if (LogManager.isVerboseLoggingEnabled()) {
    ...
    if (pduToParse == null) {
      ...
      byte[] typeCodeBytes = longToByteArray(getMatchingBeaconTypeCode(),
          mMatchingBeaconTypeCodeEndOffset - mMatchingBeaconTypeCodeStartOffset + 1);
      if (getServiceUuid() != null) {
        serviceUuidBytes =
            longToByteArray(getServiceUuid(), mServiceUuidEndOffset - mServiceUuidStartOffset + 1,
                false);
      }
      ...
      if (getServiceUuid() == null) {
        if (byteArraysMatch(bytesToProcess, startByte + mMatchingBeaconTypeCodeStartOffset,
            typeCodeBytes, 0)) {
          patternFound = true;
        }
      } else {
        if (byteArraysMatch(bytesToProcess, startByte + mServiceUuidStartOffset, serviceUuidBytes,
            0) && byteArraysMatch(bytesToProcess, startByte + mMatchingBeaconTypeCodeStartOffset,
            typeCodeBytes, 0)) {
          patternFound = true;
        }
      }
        ...
        if (getServiceUuid() == null) {
          ...
          if (LogManager.isVerboseLoggingEnabled()) {
            LogManager.d(TAG,
                ...
                startByte + mServiceUuidStartOffset, byteArrayToString(typeCodeBytes),
                startByte + mMatchingBeaconTypeCodeStartOffset, bytesToHex(bytesToProcess));
          }
        }
        parseFailed = true;
        beacon = null;
      ...
      if (patternFound) {
        if (bytesToProcess.length <= startByte + mLayoutSize && mAllowPduOverflow) {
          ...
          if (LogManager.isVerboseLoggingEnabled()) {
            LogManager.d(TAG, "Expanding buffer because it is too short to parse: "
        ...
        for (int i = 0; i < mIdentifierEndOffsets.size(); i++) {
          ...
          if (endIndex > pduToParse.getEndIndex() && mIdentifierVariableLengthFlags.get(i)) {
            if (LogManager.isVerboseLoggingEnabled()) {
              LogManager.d(TAG,
                  "Need to truncate identifier by " + (endIndex - pduToParse.getEndIndex()));
            }
            // If this is a variable length identifier, we truncate it to the size that
            // is available in the packet
            Identifier identifier =
                Identifier.fromBytes(bytesToProcess, mIdentifierStartOffsets.get(i) + startByte,
                    pduToParse.getEndIndex() + 1, mIdentifierLittleEndianFlags.get(i));
            identifiers.add(identifier);
          } else if (endIndex > pduToParse.getEndIndex() && !mAllowPduOverflow) {
            parseFailed = true;
            if (LogManager.isVerboseLoggingEnabled()) {
              LogManager.d(TAG, "Cannot parse identifier "
                  + i
            ...
            Identifier identifier =
                Identifier.fromBytes(bytesToProcess, mIdentifierStartOffsets.get(i) + startByte,
                    endIndex + 1, mIdentifierLittleEndianFlags.get(i));
            identifiers.add(identifier);
          }
        }
        for (int i = 0; i < mDataEndOffsets.size(); i++) {
          ...
          if (endIndex > pduToParse.getEndIndex() && !mAllowPduOverflow) {
            if (LogManager.isVerboseLoggingEnabled()) {
              LogManager.d(TAG, "Cannot parse data field "
                  + i
                  + " because PDU is too short.  endIndex: "
                  + endIndex
                  + " PDU endIndex: "
            ...
            String dataString =
                byteArrayToFormattedString(bytesToProcess, mDataStartOffsets.get(i) + startByte,
                    endIndex, mDataLittleEndianFlags.get(i));
        ...
        if (mPowerStartOffset != null) {
          ...
          try {
            if (endIndex > pduToParse.getEndIndex() && !mAllowPduOverflow) {
              parseFailed = true;
              if (LogManager.isVerboseLoggingEnabled()) {
                LogManager.d(TAG, "Cannot parse power field because PDU is too short.  endIndex: "
                    + endIndex
                    + " PDU endIndex: "
                    + pduToParse.getEndIndex());
              }
              ...
              String powerString =
                  byteArrayToFormattedString(bytesToProcess, mPowerStartOffset + startByte,
                      mPowerEndOffset + startByte, false);
    ...
    if (parseFailed) {
      ...
      String beaconTypeString =
          byteArrayToFormattedString(bytesToProcess, mMatchingBeaconTypeCodeStartOffset + startByte,
              mMatchingBeaconTypeCodeEndOffset + startByte, false);
   ...
   */
  @TargetApi(Build.VERSION_CODES.GINGERBREAD) public byte[] getBeaconAdvertisementData(
      Beacon beacon) {
    byte[] advertisingBytes;

    if (beacon.getIdentifiers().size() != getIdentifierCount()) {
      throw new IllegalArgumentException("Beacon has "
    ...
    for (int identifierNum = 0; identifierNum < this.mIdentifierEndOffsets.size();
        identifierNum++) {
    ...
    for (int identifierNum = 0; identifierNum < this.mIdentifierStartOffsets.size();
        identifierNum++) {
      if (mIdentifierVariableLengthFlags.get(identifierNum)) {
        int declaredIdentifierLength =
            (this.mIdentifierEndOffsets.get(identifierNum) - this.mIdentifierStartOffsets.get(
                identifierNum) + 1);
    ...
    for (int index = this.mMatchingBeaconTypeCodeStartOffset;
        index <= this.mMatchingBeaconTypeCodeEndOffset; index++) {
      byte value =
          (byte) (this.getMatchingBeaconTypeCode() >> (8 * (this.mMatchingBeaconTypeCodeEndOffset
              - index)) & 0xff);
    ...
    for (int identifierNum = 0; identifierNum < this.mIdentifierStartOffsets.size();
        identifierNum++) {
      byte[] identifierBytes = beacon.getIdentifier(identifierNum)
          .toByteArrayOfSpecifiedEndianness(!this.mIdentifierLittleEndianFlags.get(identifierNum));
      ...
      if (identifierBytes.length < getIdentifierByteCount(identifierNum)) {
        if (!mIdentifierVariableLengthFlags.get(identifierNum)) {
          ...
          if (mIdentifierLittleEndianFlags.get(identifierNum)) {
            ...
            System.arraycopy(identifierBytes, 0, newIdentifierBytes,
        ...
        LogManager.d(TAG,
            "Expanded identifier because it is too short.  It is now: " + byteArrayToString(
                identifierBytes));
      } else if (identifierBytes.length > getIdentifierByteCount(identifierNum)) {
        if (mIdentifierLittleEndianFlags.get(identifierNum)) {
          // Truncate it at the beginning for big endian
          identifierBytes = Arrays.copyOfRange(identifierBytes,
              getIdentifierByteCount(identifierNum) - identifierBytes.length,
        ...
        LogManager.d(TAG,
            "Truncated identifier because it is too long.  It is now: " + byteArrayToString(
                identifierBytes));
      } else {
    ...
    for (int dataFieldNum = 0; dataFieldNum < this.mDataStartOffsets.size(); dataFieldNum++) {
      ...
      int dataFieldLength =
          this.mDataEndOffsets.get(dataFieldNum) - this.mDataStartOffsets.get(dataFieldNum);
      for (int index = 0; index <= dataFieldLength; index++) {
        ...
        advertisingBytes[this.mDataStartOffsets.get(dataFieldNum) - 2 + endianCorrectedIndex] =
            (byte) (dataField >> (8 * index) & 0xff);
      }
    }
  ...
  public int getIdentifierByteCount(int identifierNum) {
    return mIdentifierEndOffsets.get(identifierNum) - mIdentifierStartOffsets.get(identifierNum)
        + 1;
  }
  ...
  /**
   * @return the correction value in dBm to apply to the calibrated txPower to get a 1m calibrated
   * value.
