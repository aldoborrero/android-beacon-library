...
/**
 * A class used to set up interaction with beacons from an <code>Activity</code> or
 * <code>Service</code>.
 * This class is used in conjunction with <code>BeaconConsumer</code> interface, which provides a
 * callback
 * when the <code>BeaconService</code> is ready to use.  Until this callback is made, ranging and
 * monitoring
...
public class BeaconManager {
  ...
  private final List<Region> monitoredRegions = new ArrayList<>();
  private final List<Region> rangedRegions = new ArrayList<>();
   ...
   * instead.
   */
  @Deprecated public static void setDebug(boolean debug) {
    if (debug) {
  ...
  /**
   * The default duration in milliseconds of the Bluetooth scan cycle when no ranging/monitoring
   * clients are in the foreground
   */
  public static final long DEFAULT_BACKGROUND_SCAN_PERIOD = 10000;
  /**
   * The default duration in milliseconds spent not scanning between each Bluetooth scan cycle when
   * no ranging/monitoring clients are in the foreground
   */
   ...
   * @throws BleNotAvailableException if Bluetooth LE is not supported.  (Note: The Android
   * emulator
   * will do this)
   */
  @TargetApi(18) public boolean checkAvailability() throws BleNotAvailableException {
    ...
    if (!mContext.getPackageManager().hasSystemFeature(PackageManager.FEATURE_BLUETOOTH_LE)) {
      ...
      if (((BluetoothManager) mContext.getSystemService(Context.BLUETOOTH_SERVICE)).getAdapter()
          .isEnabled()) {
        return true;
      }
    }
   ...
   * <code>BeaconService</code>.  The
   * <code>Activity</code> or <code>Service</code> must implement the <code>beaconConsumer</code>
   * interface so
  ...
  public void bind(BeaconConsumer consumer) {
    ...
    synchronized (consumers) {
      ...
      if (alreadyBoundConsumerInfo != null) {
        ...
        consumer.bindService(intent, newConsumerInfo.beaconServiceConnection,
   ...
   * <code>BeaconService</code>.  This should
   ...
   * <code>Region</code> object, and providing updates on the estimated mDistance every seconds
   * while
   * beacons in the Region are visible.  Note that the Region's unique identifier must be retained
   * to
   ...
   * @see Region
   */
  @TargetApi(18) public void startRangingBeaconsInRegion(Region region) throws RemoteException {
    if (android.os.Build.VERSION.SDK_INT < 18) {
      LogManager.w(TAG, "Not supported prior to API 18.  Method invocation will be ignored");
      return;
    }
    ...
    StartRMData obj = new StartRMData(region, callbackPackageName(), this.getScanPeriod(),
        this.getBetweenScanPeriod(), this.mBackgroundMode);
    msg.obj = obj;
   ...
   * @see Region
   */
  @TargetApi(18) public void stopRangingBeaconsInRegion(Region region) throws RemoteException {
    if (android.os.Build.VERSION.SDK_INT < 18) {
      LogManager.w(TAG, "Not supported prior to API 18.  Method invocation will be ignored");
      return;
    }
   ...
   */
  @TargetApi(18) public void updateScanPeriods() throws RemoteException {
    if (android.os.Build.VERSION.SDK_INT < 18) {
      LogManager.w(TAG, "Not supported prior to API 18.  Method invocation will be ignored");
      return;
    }
    ...
    LogManager.d(TAG, "updating scan period to %s, %s", this.getScanPeriod(),
        this.getBetweenScanPeriod());
    StartRMData obj =
        new StartRMData(this.getScanPeriod(), this.getBetweenScanPeriod(), this.mBackgroundMode);
    msg.obj = obj;
    serviceMessenger.send(msg);
  }
  ...
  private class BeaconServiceConnection implements ServiceConnection {
    ...
    public void onServiceConnected(ComponentName className, IBinder service) {
      ...
      synchronized (consumers) {
        for (Map.Entry<BeaconConsumer, ConsumerInfo> entry : consumers.entrySet()) {
