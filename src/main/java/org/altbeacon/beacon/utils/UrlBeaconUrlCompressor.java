...
public class UrlBeaconUrlCompressor {
  ...
  private static final String EDDYSTONE_URL_REGEX =
   ...
   * 1: http or https
   * 2: www. or empty
   * 3: hostname including optional leading www. but excluding trailing dot up to but not including
   * TLD
      ...
      if (urlMatcher.matches()) {
        ...
        if (protocol.equalsIgnoreCase(URL_PROTOCOL_HTTP)) {
          byteBuffer[byteBufferIndex] =
              (haswww ? EDDYSTONE_URL_PROTOCOL_HTTP_WWW : EDDYSTONE_URL_PROTOCOL_HTTP);
        } else {
          byteBuffer[byteBufferIndex] =
              (haswww ? EDDYSTONE_URL_PROTOCOL_HTTPS_WWW : EDDYSTONE_URL_PROTOCOL_HTTPS);
        }
        byteBufferIndex++;
