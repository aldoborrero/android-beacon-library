...
public class BeaconService extends Service {
  ...
  private final Map<Region, RangeState> rangedRegionState = new HashMap<>();
        ...
        switch (msg.what) {
          case MSG_START_RANGING:
            ...
            service.startRangingBeaconsInRegion(startRMData.getRegionData(),
                new org.altbeacon.beacon.service.Callback(startRMData.getCallbackPackageName()));
            service.setScanPeriods(startRMData.getScanPeriod(), startRMData.getBetweenScanPeriod(),
                startRMData.getBackgroundFlag());
            break;
  ...
  @Override public void onCreate() {
    ...
    mCycledScanner =
        CycledLeScanner.createScanner(this, BeaconManager.DEFAULT_FOREGROUND_SCAN_PERIOD,
            BeaconManager.DEFAULT_FOREGROUND_BETWEEN_SCAN_PERIOD, mBackgroundFlag,
            mCycledLeScanCallback, bluetoothCrashResolver);

    beaconManager = BeaconManager.getInstanceForApplication(getApplicationContext());
    beaconParsers = beaconManager.getBeaconParsers();
    defaultDistanceCalculator =
        new ModelSpecificDistanceCalculator(this, BeaconManager.getDistanceModelUpdateUrl());
    ...
    try {
      ...
      LogManager.e(e, TAG,
          "Cannot get simulated Scan data.  Make sure your org.altbeacon.beacon.SimulatedScanData class defines a field with the signature 'public static List<Beacon> beacons'");
    }
  }

  @Override public int onStartCommand(Intent intent, int flags, int startId) {
    LogManager.i(TAG,
        intent == null ? "starting with null intent" : "starting with intent " + intent.toString());
  ...
  @Override public void onTaskRemoved(Intent rootIntent) {
    ...
    if (Build.VERSION.RELEASE.contains("4.4.1") ||
      ...
      alarmManager.set(AlarmManager.RTC_WAKEUP, System.currentTimeMillis() + 1000,
          getRestartIntent());
      LogManager.d(TAG,
          "Setting a wakeup alarm to go off due to Android 4.4.2 service restarting bug.");
    }
  }
    ...
    synchronized (rangedRegionState) {
      if (rangedRegionState.containsKey(region)) {
        ...
        rangedRegionState.remove(
            region); // need to remove it, otherwise the old object will be retained because they are .equal //FIXME That is not true
      }
  ...
  protected final CycledLeScanCallback mCycledLeScanCallback = new CycledLeScanCallback() {
    ...
    @Override public void onCycleEnd() {
  ...
  private void processRangeData() {
    synchronized (rangedRegionState) {
      for (Region region : rangedRegionState.keySet()) {
        ...
        rangeState.getCallback()
            .call(BeaconService.this, "rangingData",
                new RangingData(rangeState.finalizeBeacons(), region));
      }
    }
  }
  ...
  private class ScanProcessor extends AsyncTask<ScanData, Void, Void> {
    ...
    @Override protected Void doInBackground(ScanData... params) {
      ...
      if (beacon != null) {
        ...
        if (mNonBeaconLeScanCallback != null) {
          mNonBeaconLeScanCallback.onNonBeaconLeScan(scanData.device, scanData.rssi,
              scanData.scanRecord);
        }
      }
      return null;
