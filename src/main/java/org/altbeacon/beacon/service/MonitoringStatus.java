...
public class MonitoringStatus {
  ...
  public synchronized void addRegion(Region region) {
    ...
    mRegionsStatesMap.put(region,
        new RegionMonitoringState(new Callback(mContext.getPackageName())));
  ...
  public synchronized void updateNewlyOutside() {
    ...
    while (monitoredRegionIterator.hasNext()) {
      ...
      if (state.isNewlyOutside()) {
        ...
        state.getCallback()
            .call(mContext, "monitoringData", new MonitoringData(state.isInside(), region));
      }
    }
  ...
  private void restoreMonitoringStatus() {
    ...
    try {
      ...
      Map<Region, RegionMonitoringState> obj =
          (Map<Region, RegionMonitoringState>) objectInputStream.readObject();
      mRegionsStatesMap.putAll(obj);
    } catch (IOException | ClassNotFoundException | ClassCastException e) {
      if (e instanceof InvalidClassException) {
        LogManager.d(TAG,
            "Serialized Monitoring State has wrong class. Just ignoring saved state...");
      } else {
