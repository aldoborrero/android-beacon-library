...
@TargetApi(Build.VERSION_CODES.LOLLIPOP) public class CycledLeScannerForLollipop
  ...
  public CycledLeScannerForLollipop(Context context, long scanPeriod, long betweenScanPeriod,
      boolean backgroundFlag, CycledLeScanCallback cycledLeScanCallback,
      BluetoothCrashResolver crashResolver) {
    super(context, scanPeriod, betweenScanPeriod, backgroundFlag, cycledLeScanCallback,
        crashResolver);
    mBeaconManager = BeaconManager.getInstanceForApplication(mContext);
  }

  @Override protected void stopScan() {
    try {
      if (getScanner() != null) {
        try {
  ...
  protected boolean deferScanIfNeeded() {
    ...
    if (millisecondsUntilStart > 0) {
      ...
      long secsSinceLastDetection =
          SystemClock.elapsedRealtime() - DetectionTracker.getInstance().getLastDetectionTime();
      ...
      if (mBackgroundLScanStartTime > 0l) {
        ...
        if (DetectionTracker.getInstance().getLastDetectionTime() > mBackgroundLScanStartTime) {
          if (mBackgroundLScanFirstDetectionTime == 0l) {
            mBackgroundLScanFirstDetectionTime =
          ...
          if (SystemClock.elapsedRealtime() - mBackgroundLScanFirstDetectionTime
            ...
            LogManager.d(TAG,
                "We've been detecting for a bit.  Stopping Android L background scanning");
            stopScan();
  ...
  @Override protected void startScan() {
  ...
  @Override protected void finishScan() {
    ...
    if (leScanCallback == null) {
      leScanCallback = new ScanCallback() {
        ...
        @Override public void onScanResult(int callbackType, ScanResult scanResult) {
          ...
          mCycledLeScanCallback.onLeScan(scanResult.getDevice(), scanResult.getRssi(),
