...
public abstract class CycledLeScanner {
  ...
  protected CycledLeScanner(Context context, long scanPeriod, long betweenScanPeriod,
  ...
  public static CycledLeScanner createScanner(Context context, long scanPeriod,
      long betweenScanPeriod, boolean backgroundFlag, CycledLeScanCallback cycledLeScanCallback,
      BluetoothCrashResolver crashResolver) {
    ...
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
      ...
      if (BeaconManager.isAndroidLScanningDisabled()) {
        LogManager.i(TAG,
            "This Android 5.0, but L scanning is disabled. We are using old scanning APIs");
        useAndroidLScanner = false;
      } else {
    ...
    if (useAndroidLScanner) {
      ...
      return new CycledLeScannerForJellyBeanMr2(context, scanPeriod, betweenScanPeriod,
          backgroundFlag, cycledLeScanCallback, crashResolver);
    }
  }
  ...
  public void setScanPeriods(long scanPeriod, long betweenScanPeriod, boolean backgroundFlag) {
    ...
    if (mNextScanCycleStartTime > now) {
      ...
      if (proposedNextScanStartTime < mNextScanCycleStartTime) {
        ...
        LogManager.i(TAG, "Adjusted nextScanStartTime to be %s", new Date(
            mNextScanCycleStartTime - SystemClock.elapsedRealtime() + System.currentTimeMillis()));
      }
    }
  ...
  @SuppressLint("NewApi") public void stop() {
  ...
  @SuppressLint("NewApi") protected void scanLeDevice(final Boolean enable) {
          ...
          if (getBluetoothAdapter() != null) {
            if (getBluetoothAdapter().isEnabled()) {
              if (mBluetoothCrashResolver != null
                  && mBluetoothCrashResolver.isRecoveryInProgress()) {
  ...
  protected BluetoothAdapter getBluetoothAdapter() {
    if (mBluetoothAdapter == null) {
      ...
      final BluetoothManager bluetoothManager = (BluetoothManager) mContext.getApplicationContext()
  ...
  protected void setWakeUpAlarm() {
    ...
    alarmManager.set(AlarmManager.ELAPSED_REALTIME_WAKEUP,
        SystemClock.elapsedRealtime() + milliseconds, getWakeUpOperation());
    LogManager.d(TAG, "Set a wakeup alarm to go off in %s ms: %s", milliseconds,
        getWakeUpOperation());
  }
  ...
  protected PendingIntent getWakeUpOperation() {
    if (mWakeUpOperation == null) {
      ...
      mWakeUpOperation =
          PendingIntent.getBroadcast(mContext, 0, wakeupIntent, PendingIntent.FLAG_UPDATE_CURRENT);
    }
  ...
  protected void cancelWakeUpAlarm() {
    ...
    LogManager.d(TAG, "Set a wakeup alarm to go off in %s ms: %s",
        milliseconds - SystemClock.elapsedRealtime(), getWakeUpOperation());
  }
  ...
  private long getNextScanStartTime() {
    ...
    long normalizedBetweenScanPeriod =
        mBetweenScanPeriod - (SystemClock.elapsedRealtime() % fullScanCycle);
  ...
  private boolean checkPermission(final String permission) {
    return mContext.checkPermission(permission, android.os.Process.myPid(),
